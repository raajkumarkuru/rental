 ...
 *   description = @Translation("Provides a form element to select a single or multiple terms displayed as an SHS element."),
...
class ShsTermSelect extends WebformTermSelect {
  ...
  public function form(array $form, FormStateInterface $form_state): array {
    ...
    $form['term_reference']['force_deepest_error'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Custom force deepest error message'),
      '#default_value' => $element_properties['force_deepest_error'] ?? '',
      '#description' => $this->t('If set, this message will be used when a user does not choose the deepest option, instead of the default "You need to select a term from the deepest level in field X." message.'),
      '#states' => [
        'visible' => [
    ...
    $form['term_reference']['cache_options'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Cache terms'),
      '#default_value' => $element_properties['cache_options'] ?? FALSE,
      '#description' => $this->t('Speeds up the loading time for Vocabularies containing many Taxonomy Terms.'),
    ];

    $form['term_reference']['addNewLabel'] = [
      '#type' => 'textfield',
      '#title' => $this->t('The label of "Add New Item" button'),
      '#default_value' => $element_properties['addNewLabel'] ?? '',
    ];

    $form['term_reference']['depth_labels'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('Depth Labels'),
      '#description' => $this->t('Customize the labels that will appear in the form element for each level of depth. Fields can be left blank for the defaults.'),
      '#access' => TRUE,
      '#tree' => TRUE,
    ...
    foreach (range(1, $deltas) as $delta) {
      $form['term_reference']['depth_labels'][$delta] = [
        ...
        '#type' => 'textfield',
        '#default_value' => $element_properties['depth_labels'][$delta - 1] ?? '',
      ];
    }
  ...
  /**
   * Submit callback for adding depth labels.
   *
   * @param array $form
   *   Form array.
   ...
   *   Form state object.
   */
  public static function addDepthLevelSubmit(array $form, FormStateInterface $form_state): void {
   ...
   *
   * @return array
   *   The depth labels form element.
   */
  public static function addDepthLevelAjax(array $form, FormStateInterface $form_state): array {
   ...
   * {@inheritdoc}
   */
  public function formatHtmlItem(array $element, WebformSubmissionInterface $webform_submission, array $options = []): string|array {
   ...
   */
  protected function getTermStorage(): \Drupal\taxonomy\TermStorageInterface {
   ...
   * {@inheritdoc}
   */
  public function getConfigurationFormProperties(array &$form, FormStateInterface $form_state): array {
   ...
   * {@inheritdoc}
   */
  protected function setOptions(array &$element): void {
