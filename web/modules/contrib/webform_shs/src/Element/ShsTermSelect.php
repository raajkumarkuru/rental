...
class ShsTermSelect extends Select {
  ...
  public static function processSelect(&$element, FormStateInterface $form_state, &$complete_form): array {
    ...
    if ($route && $admin_context->isAdminRoute($route)) {
      return $element;
    }
    ...
    $module_handler->alter($hooks, $settings_shs, $bundle, $element['#webform_key']);
  ...
  public static function setOptions(array &$element): void {
    ...
    $module_handler = \Drupal::service('module_handler');
    if (!$module_handler->moduleExists('taxonomy') || empty($element['#vocabulary'])) {
      $element['#options'] = [];
      return;
    }
   ...
   */
  protected static function getOptions(array $element): array {
  ...
  protected static function buildCachedTermOptions(string $vid): array {
    ...
    if ($cache_data = $cache->get($cid)) {
      $options = $cache_data->data;
    }
    else {
      $options = self::buildTermOptions($vid);
      $cache->set($cid, $options, Cache::PERMANENT);
    }
